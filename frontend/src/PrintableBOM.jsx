...
function PrintableBOM({ projectId: propProjectId }) {
    ...
    if (storedData) {
      try {
        ...
        if (
          ...
          (!parsedData.project?.inverter_brand_model ||
            !parsedData.project?.battery_brand_model)
        ) {
          ...
          parsedData.categories.forEach((cat) => {
            ...
            if (
              ...
              if (item.product?.brand) {
                ...
                const size = item.product?.rating_kva
                  ? `${Number(item.product.rating_kva).toFixed(0)}kVA`
                  : item.product?.power_w
                    ? `${Number(item.product.power_w / 1000).toFixed(0)}kW`
                    : "";
              ...
              if (item.product?.brand) {
                const brand = item.product.brand;
                const nominalSize = item.product?.capacity_kwh
                  ? `${(Number(item.product.capacity_kwh) / 0.8).toFixed(0)}kWh`
                  : "";
  ...
  const carryPages = useMemo(() => {
    ...
    const h = {
      totals: H_TOT_PX,
      termsDeposit: H_TERM_PX,
      bankingAccept: H_BANK_PX,
    };
    const pagesArr = [];
    let page = [];
  ...
  const renderHeader = () => (
    <header className="bom-header">
      ...
      <div className="bom-title">
        {isQuoteMode ? "Quotation" : "Bill of Materials"}
      </div>
      <div className="bom-info-grid">
        ...
        <div>
          ...
          {isQuoteMode
            ? bomData.project?.quote_number || "QTE000000"
            : `QTE${(projectData?.id || bomData.project?.id || "").toString().padStart(8, "0")}`}
        </div>
        <div>
          <span className="label">Address:</span>{" "}
          {projectData?.location || bomData.project?.location || "Address"}
        </div>
        <div>
        ...
        <div>
          ...
          {projectData?.client_email || bomData.project?.client_email || "-"}
        </div>
        <div>
          <span className="label">Email:</span>{" "}
        ...
        <div>
          ...
          {projectData?.client_phone || bomData.project?.client_phone || "-"}
        </div>
        <div>
  ...
  const renderRow = (r, idx) => {
    if (r.type === "category") {
      ...
      if (priceMode === "qty") {
        return (
          <tr className="bom-category-row" key={`cat-${idx}`}>
            <td className="bom-category-cell" colSpan={2}>
              {r.name}
            </td>
          </tr>
        );
      ...
      return (
        <tr className="bom-category-row" key={`cat-${idx}`}>
          <td className="bom-category-cell" colSpan={preCols}>
            {r.name}
          </td>
          <td
            className="bom-cell bom-col-total"
    ...
    if (priceMode === "qty") {
      return (
        <tr className="bom-row" key={`row-${idx}`}>
          ...
          <td
            className="bom-cell bom-col-units"
    ...
    return (
      <tr className="bom-row" key={`row-${idx}`}>
        <td className="bom-cell bom-col-desc">{desc}</td>
        <td className="bom-cell bom-col-units" style={{ textAlign: "center" }}>
          {item.quantity}
        </td>
        <td className="bom-cell bom-col-unitprice">
          {formatCurrency(item.price)}
        </td>
        <td className="bom-cell bom-col-total">
      ...
      <section className="bom-block bom-block-totals">
        <div className="bom-totals">
          ...
          <div className="row">
            ...
            <div className="value">
              {formatCurrency(bomData.totals?.total_excl_vat || 0)}
            </div>
          </div>
          <div className="row">
            <div className="label">{vatPerc}% VAT</div>
            <div className="value">
              {formatCurrency(bomData.totals?.vat_price || 0)}
            </div>
          </div>
          <div className="rule" />
          <div className="row grand">
            ...
            <div className="value">
              {formatCurrency(bomData.totals?.total_incl_vat || 0)}
            </div>
          </div>
      ...
      return (
        <div className="container mt-5 text-center">
          <Alert variant="warning">
            ...
            <p>
              Unable to load quote data. Please try refreshing the page or
              contact support.
            </p>
          </Alert>
        </div>
      ...
      return (
        <div className="container mt-5 text-center">
          <div className="alert alert-info">
            ...
            <p>
              Please go to the <strong>Bill of Materials</strong> tab and click{" "}
              <strong>"Generate Quote"</strong> to create a printable quote for
              this project.
            </p>
          </div>
        </div>
      ...
      >
                ...
                bg={
                  quoteData.quote.status === "sent"
                    ...
                    : quoteData.quote.status === "accepted"
                      ? "primary"
                      : quoteData.quote.status === "declined"
                        ? "danger"
                        : "secondary"
                }
              >
            ...
            {quoteData.quote.status !== "sent" &&
              ...
              quoteData.quote.status !== "declined" && (
                <Button
                  ...
                  title={
                    actionLoading ? "Sending quote..." : "Send Quote to Client"
                  }
                >
                  {actionLoading ? (
                    <>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            ...
            >
              <Badge
                bg={termsSum === 100 ? "success" : "warning"}
            ...
            <div className="bom-content">
              {pg.kind === "rows" && (
                <table className="bom-table">
                  {renderTableHead()}
                  <tbody>{pg.rows.map((r, i) => renderRow(r, i))}</tbody>
                </table>
              )}
              ...
              {pg.kind === "rows" && pageIndex === pages.length - 1 && (
                <>
                  ...
                  {totalsPlacement.inlineKeys.includes("termsDeposit") && (
                    <TermsDepositBlock />
                  )}
                  {totalsPlacement.inlineKeys.includes("bankingAccept") && (
                    <BankingAcceptanceBlock />
                  )}
                </>
              )}
              ...
              {pg.kind === "totals" && (
                <>
                  ...
                  {pg.blocks.includes("bankingAccept") && (
                    <BankingAcceptanceBlock />
                  )}
                </>
              )}
