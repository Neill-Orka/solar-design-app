...
function ReportBuilder({ projectId, onNavigateToTab }) {
  ...
  const [reportSettings, setReportSettings] = useState({
    ...
    paymentSchedule: [
      { milestone: "Deposit", amount: "65" },
      { milestone: "Delivery", amount: "25" },
      { milestone: "Handover", amount: "10" },
    ],
    projectSchedule: [
      {
        activity: "Deposit received & document compilation",
        timeline: "Week 1",
      },
      {
  ...
  const checkRequiredData = (data) => {
    ...
    if (
      ...
      data.simulation.timestamps.length === 0
    ) {
      missing.push({
        type: "simulation",
  ...
  const calculatePageCounts = () => {
    ...
    let totalPages = Object.values(pageCounts).reduce(
      (sum, count) => sum + count,
      0
    );
    let currentPage = 1;
    ...
    return {
      ...
      executiveSummary: {
        start: currentPage,
        count: pageCounts.executiveSummary,
      },
      designReportMeta: {
        start: (currentPage += pageCounts.executiveSummary),
        count: pageCounts.designReportMeta,
      },
      mainReportContent: {
        start: (currentPage += pageCounts.designReportMeta),
        count: pageCounts.mainReportContent,
      },
      bomSection: {
        start: (currentPage += pageCounts.mainReportContent),
        count: pageCounts.bomSection,
      },
    };
  };
  ...
  useEffect(() => {
    ...
    const simulationResults = JSON.parse(
      sessionStorage.getItem(`simulationData_${projectId}`) || "{}"
    );
    const financialResults = JSON.parse(
      sessionStorage.getItem(`financialResult_${projectId}`) || "{}"
    );
      ...
      .then((projectRes) => {
        ...
        const getBrandModel = (products, ids, category) => {
          ...
          const prod = products.find(
            (p) => ids.includes(p.id) && p.category === category
          );
        ...
        project.inverter_brand_model = getBrandModel(
          products,
          project.inverter_ids,
          "Inverter"
        );
        project.battery_brand_model = getBrandModel(
          products,
          project.battery_ids,
          "Battery"
        );

        project.battery_nominal_rating =
          project.battery_kwh && project.battery_kwh.capacity > 0
            ? Number(
                getNominalRating(products, project.battery_ids, "Battery") *
                  project.battery_kwh.quantity
              ).toFixed(0)
            : 0;
  ...
  return (
    <div className={`report-builder ${sidebarOpen ? "sidebar-open" : ""}`}>
        ...
        <div className="section-toggle">
            ...
            <input
              ...
              onChange={() =>
                setReportSettings((prev) => ({
                  ...prev,
                  usedActualConsumption: !prev.usedActualConsumption,
                }))
              }
            />
          ...
          <label>
            <input
              ...
              onChange={() =>
                setSections((s) => ({ ...s, siteLayout: !s.siteLayout }))
              }
            />
            Show Site Layout
          </label>
          ...
          <label>
            <input
              ...
              onChange={() =>
                setReportSettings((prev) => ({
                  ...prev,
                  generatorIncluded: !prev.generatorIncluded,
                }))
              }
            />
          ...
          <div>
            <h6>Adjust Project Schedule</h6>
            <h7>
              <u>Payment timeline</u>
            </h7>
            <br />
            {reportSettings.paymentSchedule.map((item, index) => (
              <div key={index}>
                ...
                <input
                  ...
                  value={item.amount}
                  onChange={(e) => {
                    const newSchedule = [...reportSettings.paymentSchedule];
      ...
      <button
        className="orka-sidebar-export-btn"
        ...
        {loading ? (
          ...
          <>
            ...
            <ExecutiveSummary
              ...
              pageNumber={pageNumbers.executiveSummary.start}
            ...
            {sections.bom && (
              <BOMSection
                data={data}
                settings={reportSettings}
                startPageNumber={pageNumbers.bomSection.start}
                totalPages={pageNumbers.totalPages}
              />
            )}
          </>
