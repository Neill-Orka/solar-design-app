...
function MainReportContent({
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_URL}/api/consumption_data/${data.project.id}`, {
        params: {
  ...
  const totalKwh =
    consumptionData.length > 0
      ? (
          consumptionData.reduce((sum, row) => sum + row.demand_kw, 0) * 0.5
        ).toFixed(0)
      : 0;
  const dates = new Set(
    consumptionData.map((row) => row.timestamp?.split("T")[0] || "")
  );
  const avgDailyKwh = dates.size > 0 ? (totalKwh / dates.size).toFixed(0) : 0;
  const peakDemand =
    consumptionData.length > 0
      ? Math.max(...consumptionData.map((row) => row.demand_kw)).toFixed(1)
      : 0;
  const avgDemand =
    consumptionData.length > 0
      ? (
          consumptionData.reduce((sum, row) => sum + row.demand_kw, 0) /
  ...
  const costBreakdownChartData = React.useMemo(() => {
    if (
      !data?.financials?.cost_comparison ||
      !data.financials.cost_comparison?.length
    )
      return {
        labels: [],
        datasets: [],
  ...
  const getFilteredSimulationData = React.useMemo(() => {
    ...
    return {
      labels: data.simulation.timestamps
      ...
      import_from_grid: data.simulation.import_from_grid.slice(
        startIndex,
        endIndex + 1
      ),
  ...
  const displayValue = (value, fallback, field = "") => {
    ...
    if (typeof value === "object") {
      ...
      return (
        value.capacity || value.quantity || Object.values(value)[0] || fallback
      );
    }
    return value;
  };
  ...
  const systemContent = {
    grid: {
      projectGoal:
        "The goal of this project is to reduce grid dependency and electricity costs through a grid-tied solar PV system.",
      benefits: [
      ...
      systemDescription:
        "This grid-tied solar PV system is designed to work in conjunction with the utility grid. It produces electricity during daylight hours, reducing dependency on utility power when the sun is shining.",
    },
    hybrid: {
      projectGoal:
        "The goal of this project is to provide both energy cost savings and backup power during outages.",
      benefits: [
      ...
      systemDescription:
        "This hybrid solar PV system combines the benefits of grid-tied operation with battery storage for backup power.",
    },
    off_grid: {
      projectGoal:
        "The goal of this project is to achieve complete energy independence from the utility grid.",
      benefits: [
      ...
      systemDescription:
        "This off-grid solar PV system with battery storage is designed to meet all of the site's energy needs without connection to the utility grid.",
    },
    default: {
      projectGoal:
        "The goal of this project is to provide a custom solar energy solution tailored to the client's specific requirements, optimizing energy production while ensuring reliability and performance.",
      benefits: [
        "Reduced electricity bills",
        "Increased energy independence",
        "Lower carbon footprint",
      ...
      systemDescription:
        "This solar PV system is designed to meet the specific energy needs of the site, taking into account local conditions, consumption patterns, and future requirements.",
    },
  };
  ...
  useEffect(() => {
    if (demandStartDate) {
      sessionStorage.setItem(
        "reportDemandStartDate",
        demandStartDate.toISOString()
      );
    }
  }, [demandStartDate]);
  ...
  useEffect(() => {
    if (simulationStartDate) {
      sessionStorage.setItem(
        "reportSimulationStartDate",
        simulationStartDate.toISOString()
      );
    }
  }, [simulationStartDate]);
  ...
  useEffect(() => {
    ...
    console.log(
      `Fetching demand data from ${formattedStartDate} to ${formattedEndDate}`
    );
  ...
  useEffect(() => {
    ...
    console.log(
      `Fetching yearly data from ${formattedStartDate} to ${formattedEndDate}`
    );
  ...
  const monthlyProductionChartData = React.useMemo(() => {
    ...
    data?.simulation,
    data?.financials?.cost_comparison,
    data?.financials?.total_generation_kwh,
  ]);
  ...
  const [projectSchedule, setProjectSchedule] = useState([
    ...
    {
      activity: "Equipment procurement & delivery",
      timeline: "Week 2 (provided no major supplier delays)",
    },
  ...
  const addScheduleItem = () => {
    setProjectSchedule([...projectSchedule, { activity: "", timeline: "" }]);
  };
  ...
  return (
    <>
      ...
      >
        ...
        <div>
          This site is located at {data?.project?.location}, South Africa:{" "}
        ...
        {showSiteLayout && (
          <div className="site-layout-section mt-4">
            ...
            <p>
              The proposed module layout is shown below, this may change after
              detailed site visit but has in principle been proposed to the
              client:
            </p>
            {siteLayoutImage ? (
              ...
              <div className="no-layout-image">
                <p className="text-muted fst-italic">
                  {" "}
                  No layout image available. Please upload one in the sidebar.
                </p>
              </div>
            )}
          </div>
      ...
      >
        ...
        <h5 className="fw-bold">
          Current load profile and electrical consumption
        </h5>
        {/* Monthly energy consumption chart */}

        <p className="mt-3">
          The chart below shows the monthly energy consumption for{" "}
          {data?.project?.client_name}'s facility. This data was used as the
          foundation in the design of an optimal solar solution tailored to the
          actual usage patterns of the site.
        </p>
      ...
      >
        ...
        <p>
          The chart below shows the monthly electricity costs without solar. The
          cost can be divided into an energy and fixed cost component. This
          analysis helps identify the areas where the solar solution will have
            ...
            <p>
              {(() => {
                ...
                const avgMonthlyCost =
                  oldCosts.length > 0
                    ? oldCosts.reduce((sum, cost) => sum + cost, 0) /
                      oldCosts.length
                    : 0;

                // Maximum monthly cost
                const maxMonthlyCost =
                  oldCosts.length > 0 ? Math.max(...oldCosts) : 0;
              ...
              <br />
              This equates to an annual electricity cost of approximately{" "}
              <strong>
                R {formatValue(data.financials.original_annual_cost)}
              </strong>{" "}
              (excl. VAT).
            </p>
          </div>
      ...
      >
        ...
        <p>
          The client's typical diurnal load profile is shown in the graph below.
          This visualization helps understand hourly usage patterns and identify
          opportunities for peak saving and load management.
        </p>
        ...
        <div className="mt-2">
          <p className="mb-1 ">
            <strong>Before Project:</strong> The table below summarizes the
            current consumption and cost as before the project:
          </p>
          ...
          <div className="table-responsive">
            <table className="table table-bordered table-sm compact-table">
              ...
              <tbody className="small">
                <tr>
                  <td colSpan="2" className="fst-italic py-1">
                    All values below are for p.a. for year 1 of simulation,
                    excl. VAT, unless specifically stated otherwise
                  </td>
                </tr>
                <tr>
                  <td className="py-1">Electricity requirement p.a. (kWh)</td>
                  <td className="text-end py-1">
                    {formatValue(data?.financials?.total_demand_kwh || 0)}
                  </td>
                </tr>
                <tr>
                ...
                <tr>
                  <td className="py-1">
                    <strong>Total Cost of Electricity</strong>
                  </td>
                  <td className="text-end py-1">
                    <strong>
                      R{" "}
                      {formatValue(data?.financials?.original_annual_cost || 0)}
                    </strong>
                  </td>
                </tr>
                <tr>
                  ...
                  <td className="text-end py-1">
                    R{" "}
                    {(
                      (data?.financials?.original_annual_cost || 0) /
                      (data?.financials?.total_demand_kwh || 0)
                    ).toFixed(2)}{" "}
                    /kWh
                  </td>
                </tr>
                <tr>
                  <td className="py-1" style={{ backgroundColor: "#f8f9fa" }}>
                    <strong>Grid Supply</strong>
                  </td>
                  <td style={{ backgroundColor: "#f8f9fa" }}></td>
                </tr>
                <tr>
                  <td className="py-1">
                    Grid Electricity Consumption (y1) (kWh)
                  </td>
                  <td className="text-end py-1">
                    {formatValue(data?.financials?.total_demand_kwh || 0)}
                  </td>
                ...
                <tr>
                  ...
                  <td className="text-end py-1">
                    R {formatValue(data?.financials?.original_annual_cost || 0)}
                  </td>
                </tr>
                <tr>
          ...
          <p className="mt-2">
            Grid electricity cost shows what it would have been, if there was a
      ...
      >
        ...
        <p>This section covers the design process and technical data used.</p>

        <h5 className="fw-bold">Meteorological Data</h5>
        <p>
          The detailed designs were done using our verified proprietary software
          which takes into account the site's specific meteorological
          information to predict the typical generation that can be expected
          from the specified equipment simulated over 365 days of the year. This
          enables an accurate analysis of the system yields and in return
          accurate results on the financial returns.
        </p>
        <Image width={300} src={saGHIMap} alt="Meteorological Data" />
        <Image
          align="right"
          width={375}
          src={meanAnnualRainfall}
          alt="Rainfall Data"
        />
      </StandardPage>
      ...
      >
        ...
        <p>
          Based on the design and simulation results, the system specifications
          and financial metrics are as follows:
        </p>

        {/* Technical Specifications Banner */}
        <div
          className="system-specs-banner"
          style={{
            background: "#cddae6ff",
            padding: "20px 0",
            marginTop: "20px",
        ...
        >
          <div
            className="spec-items-container"
            style={{
              display: "flex",
              flexWrap: "wrap",
          ...
          >
            ...
            >
              ...
              <div style={{ fontSize: "14px" }}>
                {(data?.project?.panel_kw).toFixed(2) || 0}{" "}
                <span style={{ fontSize: "14px" }}>kWp</span>
              </div>
              <div style={{ fontSize: "14px" }}>
                {data?.project?.num_panels ||
                  Math.ceil(((data?.project?.panel_kw || 0) * 1000) / 565)}{" "}
                panels
              </div>
            </div>
            ...
            >
              ...
              <div style={{ fontSize: "15px" }}>
                ...
                <span style={{}}>
                  {formatValue(
                    displayValue(
                      data?.project?.inverter_kva,
                      85,
                      "inverter_kva"
                    )
                  )}{" "}
                  <span style={{ fontSize: "14px" }}>kVA</span>
                </span>
              </div>
              {/* <div style={{ fontSize: '15px' }}>
                            Hybrid{' '}
            ...
            >
              ...
              <div style={{ fontSize: "15px" }}>
                <span style={{ fontWeight: "" }}>
                  {data.project.battery_nominal_rating
                    ? formatValue(data.project.battery_nominal_rating)
                    : formatValue(
                        displayValue(
                          data?.project?.battery_kwh,
                          0,
                          "battery_kwh"
                        )
                      )}
              ...
              <div style={{ fontSize: "15px" }}>
                <span style={{ fontWeight: "" }}>
                  {formatValue(
                    data?.project?.battery_kwh_80 ||
                      displayValue(data?.project?.battery_kwh, 0, "battery_kwh")
                  )}
                </span>{" "}
            ...
            >
              ...
              <div style={{ fontSize: "14px" }}>kWh/kWp/year</div>
            </div>
            ...
            >
              ...
              <div style={{ fontSize: "14px" }}>kWh/year</div>
            </div>
        ...
        <div className="mt-3">
          ...
          <p className="mb-3">
            Combined installation capacity and yields are shown in the tables
            below:
          </p>
              ...
              <tbody className="small">
                <tr>
                  ...
                  <td className="text-end">
                    {(data?.project?.panel_kw || 0).toFixed(2)}
                  </td>
                  <td>kWp</td>
                </tr>
                <tr>
                  ...
                  <td className="text-end">
                    {data?.project?.num_panels ||
                      Math.ceil(
                        ((data?.project?.panel_kw || 0) * 1000) / 565
                      ) ||
                      0}
                  </td>
                  <td>ea</td>
                ...
                <tr>
                  ...
                  <td className="text-end" colSpan="2">
                    {settings.threePhase ? "Three Phase" : "Single Phase"}
                  </td>
                </tr>
                <tr>
                  <td>Monitoring</td>
                  <td className="text-end" colSpan="2">
                    Smart metered, remote
                  </td>
                </tr>
                <tr>
      ...
      >
        <p className="mb-3">
          The table below shows the simulated performance yields from the
          system:
        </p>
          ...
          <table className="table table-bordered table-sm">
            ...
            <tbody className="small">
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.simulation?.annual_metrics?.daytime_consumption_pct ||
                      0
                  )}
                </td>
                <td>%</td>
              ...
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.simulation?.annual_metrics?.pv_utilization_pct || 0
                  )}
                </td>
                <td>%</td>
              </tr>
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.simulation?.annual_metrics?.consumption_from_pv_pct ||
                      0
                  )}
                </td>
                <td>%</td>
              ...
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.simulation?.annual_metrics?.potential_gen_daily_kwh ||
                      0
                  )}
                </td>
                <td>kWh</td>
              ...
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.simulation?.annual_metrics?.utilized_gen_daily_kwh ||
                      0
                  )}
                </td>
                <td>kWh</td>
              ...
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.simulation?.annual_metrics
                      ?.throttling_losses_daily_kwh || 0
                  )}
                </td>
                <td>kWh</td>
              </tr>
              ...
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.simulation?.annual_metrics?.utilized_gen_annual_kwh ||
                      0
                  )}
                </td>
                <td>kWh</td>
              ...
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.simulation?.annual_metrics
                      ?.throttling_losses_annual_kwh || 0
                  )}
                </td>
                <td>kWh</td>
              </tr>
              ...
              <tr>
                ...
                <td className="text-end">
                  {data?.simulation?.annual_metrics?.battery_cycles_annual ||
                    "-"}
                </td>
                <td>cycles/y</td>
              </tr>
      ...
      >
        ...
        <p>
          The below graph shows the potential solar production from the system
          per month.
        </p>
        ...
        <p className="mt-3">
          The system is projected to generate approximately{" "}
          {formatValue(data?.financials?.potential_generation_kwh || 0)} kWh per
          year.
        </p>
        ...
        <div className="mb-4">
          <p>
            The below graph shows the consumption and generation profiles for
            the different components of the system, post implementation for a
          ...
          <div className="chart-container" style={{ height: "320px" }}>
            {data?.simulation?.timestamps ? (
              <Line
                ...
                options={{
                  ...
                  plugins: {
                    ...
                    title: {
                      display: true,
                      text: "System Performance Simulation (Sample Week)",
                    },
                    tooltip: {
                      mode: "index",
                      intersect: false,
              ...
              <div className="text-center text-muted p-5">
                No simulation data available. Please run a system simulation
                first.
              </div>
            )}
          </div>
      ...
      >
        ...
        <p>
          The key metrics to evaluate the business case of the project are shown
          in the table below:
        </p>
        ...
        <div className="table-responsive">
          <p className="small mb-1">
            Table 4: Project Business Case Metrics and Benchmarking
          </p>
          <table className="table table-bordered table-sm">
            <thead>
              <tr>
              ...
              <tr>
                <td colSpan="2" className="fst-italic small bg-light">
                  All values below are year 1 of simulation, excl. VAT.
                </td>
              </tr>
            </thead>
            <tbody className="small">
              <tr>
                ...
                <td className="text-end">
                  R {formatValue(data?.project?.project_value_excl_vat || 0)}
                </td>
              </tr>
              <tr>
                ...
                <td className="text-end">
                  R{" "}
                  {formatValue(
                    data?.project?.project_value_excl_vat * 0.73 || 0
                  )}
                </td>
              </tr>
              {/* <tr>
              ...
              {data?.project?.system_type === "off-grid" && (
                <tr>
                  ...
                  <td className="text-end">
                    R{" "}
                    {formatValue(data?.financials?.generator_running_cost || 0)}
                  </td>
                </tr>
              )}
              ...
              <tr>
                ...
                <td className="text-end">
                  R {formatValue(data?.financials?.annual_savings || 0)}
                </td>
              </tr>
              <tr>
                ...
                <td className="text-end">
                  {data?.financials?.yield_year1 ||
                    Math.round(
                      (data?.financials?.annual_savings /
                        data?.project?.project_value_excl_vat) *
                        100
                    ) ||
                    0}
                  %
              ...
              <tr>
                <td>Payback (years)</td>
                <td className="text-end">
                  {typeof data.financials?.payback_period === "number"
                    ? data.financials.payback_period.toFixed(1)
                    : data.financials.payback_period}
                </td>
              </tr>
            </tbody>
          </table>
        ...
        <p className="mt-3">
          The client's new monthly cost of electricity compared the current
          cost, as well as the cost and savings in year one after installation
        ...
        <div className="figures-side-by-side mt-4">
          <div className="figure-left">
            <p className="small mb-1">
              Figure 12: Client new monthly utility cost
            </p>
            <div className="chart-container" style={{ height: "300px" }}>
              {data?.financials?.cost_comparison?.length > 0 ? (
                <Line
                    ...
                    datasets: [
                      {
                        ...
                        data: data.financials.cost_comparison.map(
                          (item) => item.old_cost
                        ),
                        borderColor: "#6c757d",
                        borderWidth: 2.5,
          ...
          <div className="figure-right">
            ...
            <div className="chart-container" style={{ height: "300px" }}>
              {data?.financials?.original_annual_cost &&
              data?.financials?.new_annual_cost ? (
                <Bar
                  data={{
        ...
        <div className="table-responsive">
          ...
          <table className="table table-bordered table-sm">
            <thead>
              ...
              <tr>
                <td colSpan="2" className="fst-italic small bg-light">
                  All values below are for p.a. for year 1 of simulation, excl.
            ...
            <tbody className="small">
              ...
              <tr>
                ...
                <td className="text-end">
                  R {formatValue(data?.financials?.new_annual_cost || 0)}
                </td>
              </tr>
              <tr>
                ...
                <td className="text-end">
                  R{" "}
                  {(
                    (data?.financials?.new_annual_cost || 0) /
                    (data?.financials?.total_demand_kwh || 1)
                  ).toFixed(2)}
                </td>
              </tr>
              ...
              <tr>
                <td>
                  <strong>Total Cost Savings (y1)</strong>
                </td>
                <td className="text-end">
                  R {formatValue(data?.financials?.annual_savings || 0)}
                </td>
                ...
                project_type === "Commercial" && (
                  <tr>
                    ...
                    <td className="text-end">
                      R{" "}
                      {formatValue(
                        data?.financials?.generator_cost_savings || 0
                      )}
                    </td>
                  </tr>
                )}
              ...
              {data?.project?.system_type !== "off-grid" && (
                <>
                  <tr className="bg-light">
                    <td colSpan="2" className="bg-light">
                      <strong>New Grid Supply</strong>
                    </td>
                  </tr>
                  <tr>
                    <td>Grid Electricity Consumption (y1) (kWh)</td>
                    <td className="text-end">
                      {formatValue(data?.financials?.total_import_kwh || 0)}
                    </td>
                  </tr>
                  <tr>
              ...
              <tr>
                ...
                <td className="text-end">
                  {formatValue(
                    data?.financials?.pv_direct_consumption ||
                      data?.financials?.total_generation_kwh * 0.6 ||
                      0
                  )}
                </td>
              </tr>
                ...
                <>
                  ...
                  {data?.project?.allow_export && (
                    <tr>
                      ...
                      <td className="text-end">
                        {formatValue(data?.financials?.exported_energy || 0)}
                      </td>
                    </tr>
                  )}
                  <tr>
                    ...
                    <td className="text-end">
                      {formatValue(
                        data?.financials?.battery_discharge ||
                          data?.financials?.total_generation_kwh * 0.4 ||
                          0
                      )}
                    </td>
                  </tr>
              ...
              {(data?.project?.system_type === "hybrid" ||
                ...
                project_type === "Commercial" && (
                  <>
                    <tr className="bg-light">
                      <td colSpan="2">
                        <strong>New Diesel Generator</strong>
                      </td>
                    </tr>
                    <tr>
                      <td>Electricity from generator (y1) (kWh)</td>
                      <td className="text-end">
                        {formatValue(data?.financials?.generator_energy || 0)}
                      </td>
                    </tr>
                    <tr>
                      ...
                      <td className="text-end">
                        R{" "}
                        {formatValue(
                          data?.financials?.generator_running_cost || 0
                        )}
                      </td>
                    </tr>
                  </>
          ...
          {(data?.project?.system_type === "hybrid" ||
            ...
            project_type === "Commercial" && (
              <p className="small mt-2 fst-italic">
                *Generator costs based on simulated load shedding scenarios and
                quiet hours.
              </p>
            )}
        </div>
      ...
      >
        <div className="scope-of-work">
          ...
          <p>
            The following scope of work will be completed as part of the project
            and are included in the cost of the proposal:
          </p>

          <ul className="ps-4">
            <li>Engineering design done by qualified Orka Solar engineers.</li>
            <li>
              Full <span className="fw-bold">turn key</span> solution, done by
              experienced Orka Solar employees, including:
            ...
            <li>
              Manufacturer's warranty on all major equipment and workmanship
              warranty.
            </li>
            <li>
              Orka Solar warranty on installation and project performance.
            </li>
          </ul>
          ...
          <p>
            Orka Solar is product agnostic and can therefore supply equipment to
            the client's specification, provided that the equipment meets the
            minimum quality standards of our company. Orka Solar has long
            standing relationships with our product suppliers and the products
            proposed as part of this design are of the highest quality and comes
          ...
          <ul className="ps-4">
            <li>
              PV modules: <span className="fw-bold">12-year</span> product
              warranty, <span className="fw-bold">25-year</span> linear power
              output warranty
            </li>
            <li>
              Inverter: <span className="fw-bold">5-year</span> manufacturer's
              warranty (option to extend to 10 years)
            </li>
            <li>
              Mounting system <span className="fw-bold">10-year</span>{" "}
              manufacturer's warranty
            </li>
            <li>
              Workmanship warranty of <span className="fw-bold">2-years</span>
            </li>
            <li>
              Post installation maintenance, monitoring and control service
              available from Orka Solar to ensure optimal benefit from the
              system.
            </li>
          </ul>
        </div>
      ...
      >
        <div className="terms-of-service">
          ...
          <p>
            {(() => {
              const paymentDescriptions = [
                "deposit on order",
                "on delivery of panels and inverters to site",
                "on project handover certificate sign off",
              ];

              const activePayments = settings.paymentSchedule
                .map((item, index) => ({
                  amount: parseFloat(item.amount),
                  description: paymentDescriptions[index],
                }))
                .filter((item) => item.amount > 0);

              if (activePayments.length === 0) {
                return "The project's invoicing schedule will be determined upon agreement.";
              }

              let sentence = "The project's invoicing will be as follows: ";
              sentence += activePayments
                .map((item, index) => `${item.amount}% ${item.description}`)
                .join(", ")
                .replace(/,([^,]*)$/, " and$1"); // Replaces the last comma with ' and'

              return sentence + ".";
            })()}{" "}
            Prices are valid to {priceValidUntil}. Thereafter, prices may change
            due to exchange rate fluctuations, inflation (CPI) and/or supplier
            price changes. Invoices will be paid on 7 days terms. Late delivery
            penalty due to delays caused by the contractor will be 0.5% of
            project value per week.
          </p>
          <h5 className="fw-bold">Project Schedule</h5>

          <p>
            The project timeline is dependent on the date of deposit and
            availability of key components. An indicative timeline for this
            project without any delays due to long lead items are shown below:
          </p>
          ...
          <p>
            Orka Solar installations are completed in compliance with the
            following standards and regulations where applicable:
          </p>
          ...
          <ul className="ps-4">
            ...
            <li>
              NRS097-2-3: Simplified utility connection criteria for low-voltage
              connected generators
            </li>
            <li>
              South African Renewable Power Plant Grid Code (although the NRS
              097-2 series cover most issues relevant to SSEG)
            </li>
            <li>NRS 048: Electricity Supply – Quality of Supply</li>
            <li>
              SANS 10142-1 and 10142-1-2: The wiring of premises (as amended and
              published)
            </li>
            <li>
              SANS 62305-4:2011 Protection against lightning and Surge – Surge
              Arrestors and Earthing Regulations
            </li>
            <li>
              SANS 474 / NRS 057: Code of Practice for Electricity Metering
            </li>
            <li>
              Municipal Electricity Supply by-laws of the applicable
              municipality
            </li>
          </ul>
        </div>
      ...
      >
        ...
        <p>
          The following assumptions and exclusions were made and form part of
          this proposal:
        </p>
        <ol className="ps-4">
          <li>
            {settings.usedActualConsumption
              ? "Actual consumption data was used for the design and simulations of the solution."
              : "Actual consumption data was not used for the design and simulations of the solution."}{" "}
            Should the loads change or differ from the data analysed Orka Solar
            should be notified by the client.
          </li>
          <li>
            It was assumed that the consumption data is a fair representation of
            the future consumption.
          </li>
          <li>
            When applicable, in most municipalities there is currently either no
            regulations around exporting electricity to the grid, or such export
            is prohibited. For this reason, Orka Solar advise against exporting
            excess solar energy to the grid without the required permits,
            although many of Orka Solar' clients have chosen to export where
            possible. All inverters supplied by Orka Solar conform to
            internationally recognized grid-export safety regulations.
          </li>
          <li>
            The scope and activities listed as part of this proposal is
            sufficient so successfully delivery the project. Any additional
            work, not listed as part of this scope and which could not be taken
            into account, will be negotiated separately, regarding any
            additional time and/or materials that may be required.
          </li>
          <li>
            Orka Solar reserve the right to requote should a detailed site visit
            and logging not have been completed or if any of the assumptions or
            information provided are found not to be accurate.
          </li>
          <li>
            It was assumed that all existing infrastructure on site are
            installed according to the relevant standards and complies with
            regulation.
          </li>
          <li>
            ...
            <ol type="a" className="ps-4">
              <li>
                Generator integration is{" "}
                {settings.generatorIncluded ? "included" : "excluded"}.
              </li>
              <li>
                No roof inspection for possible roof mounted panels were
                included, it is recommended that the client conduct such an
                inspection or contact Orka Solar to facilitate an inspection
              ...
              <li>
                No cost for building an inverter enclosure has been excluded.
              </li>
              <li>
                No cost for internet supply has been allowed for, as the client
                will supply internet to the inverter enclosure by means of an
                ethernet socket.
              </li>
              <li>
                No costs for Eskom's SSEG or NERSA administration has been
                allowed.
              </li>
              <li>
                It was assumed that all existing infrastructure on site are
          ...
          <li>
            Orka Solar guarantees the production output of the system if an
            active Orka Solar SLA is in place.
          </li>
        </ol>
      </StandardPage>
    </>
