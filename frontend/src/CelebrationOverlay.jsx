...
export default function CelebrationOverlay({ show, onClose }) {
  ...
  // Fire confetti bursts for 10 seconds
  const startConfetti = () => {
    ...
    const frame = () => {
      ...
      cleanups.current.push(requestAnimationFrame(frame));
    };
    frame();
  };
  ...
  const startAudio = async () => {
    ...
    const sfx = new Howl({
      src: ["/celebrate/In the Jungle.mp3"],
      volume: 1,
    });
    sfx.play();
    soundsRef.current.push(sfx);
  ...
  useEffect(() => {
    ...
    // Automatically trigger onClose after 10 seconds to unmount the component
    const closeTimeout = setTimeout(() => {
      onClose();
    }, 10000);

    // Add the timeout to the cleanup array
    cleanups.current.push(() => clearTimeout(closeTimeout));

    // Return the cleanup function
    return stopEverything;
  }, [show, onClose]);

  // This component no longer renders any visible, blocking UI
  return null;
}
