...
function EditProject() {
  ...
  const handleSubmit = (e) => {
    ...
    // Create a clean copy with only the fields we actually want to update
    const updatePayload = {
      name: project.name,
    ...
    axios
      .put(`${API_URL}/api/projects/${id}`, updatePayload)
      .then(() => {
            ...
            <Card className="shadow-lg border-0 rounded-xl p-4 p-md-5">
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Card className="border-light mb-4">
                  <Card.Header className="bg-light border-0">
                    <h5 className="mb-0 fw-semibold">
                      <i className="bi bi-clipboard-data me-2"></i>Project
                      Information
                    </h5>
                  </Card.Header>
                    ...
                    <Form.Group>
                      ...
                      <Card bg="light" text="dark" className="p-3 border">
                        {project.tariff_details ? (
                          ...
                          <div>
                            <h5 className="mb-1">
                              {project.tariff_details.name}{" "}
                              <span className="text-muted">
                                ({project.tariff_details.matrix_code})
                              </span>
                            </h5>
                            <div className="mb-2">
                              <Badge pill bg="primary" className="me-1">
                                {project.tariff_details.power_user_type}
                              </Badge>
                              <Badge pill bg="info" text="dark">
                                {project.tariff_details.structure}
                              </Badge>
                            </div>
                            <Table striped bordered size="sm" className="mb-0">
                              ...
                              <tbody>
                                {project.tariff_details.rates
                                  .slice(0, 3)
                                  .map((rate, index) => (
                                    <tr key={index}>
                                      <td>{rate.charge_name}</td>
                                      <td>
                                        <strong>{rate.rate_value}</strong>{" "}
                                        {rate.rate_unit}
                                      </td>
                                    </tr>
                                  ))}
                            ...
                            {project.tariff_details.rates.length > 3 && (
                              <small className="text-muted">
                                ...and {project.tariff_details.rates.length - 3}{" "}
                                more rates.
                              </small>
                            )}
                          </div>
                          ...
                          <div>
                            ...
                            <Badge pill bg="success">
                              {project.custom_flat_rate} c/kWh
                            </Badge>
                          </div>
                        ) : (
                          // Display if no tariff is set (no change here)
                          <p className="mb-0 text-muted">
                            No tariff selected for this project.
                          </p>
                        )}
                      </Card>
                    </Form.Group>
                ...
                <Card className="border-light mb-4">
                  <Card.Header>
                    <h5 className="mb-0 fw-semibold">
                      <i className="bi bi-receipt-cutoff me-2"></i>Tariff
                      Configuration
                    </h5>
                  </Card.Header>
                  <Card.Body className="p-4">
