...
const ProductList = ({
  ...
  const filteredProducts = products.filter((p) => {
    ...
    const productText =
      `${p.brand || ""} ${p.model || ""} ${p.power_w || ""} ${p.rating_kva || ""} ${p.capacity_kwh || ""}`.toLowerCase();

    // Split search into fragments and check if all fragments exist in the product text
    const fragments = searchFilter
      .toLowerCase()
      .split(/\s+/)
      .filter((f) => f.trim());
                ...
                <Col key={product.id}>
                  <Card className="h-100 shadow-sm border-light">
                    <Card.Body className="p-3">
                      ...
                      <Card.Title className="text-md font-bold text-gray-800">
...
function SystemBuilder() {
  ...
  const applySmartSearch = (product) => {
    ...
    const searchableText =
      `${product.brand || ""} ${product.model || ""} ${product.category || ""} ${product.power_w || ""} ${product.rating_kva || ""} ${product.capacity_kwh || ""}`.toLowerCase();

    // Split search into fragments and check if all exist in searchable text
    const fragments = mainSearch
      .toLowerCase()
      .split(/\s+/)
      .filter((f) => f.trim());
  ...
  const { totalCost, totalPanelKw, totalInverterKva, totalBatteryKwh } =
    useMemo(() => {
      ...
      components.forEach((comp) => {
        ...
        if (comp.product.category === "panel")
          pKw += ((comp.product.power_w || 0) * qty) / 1000;
        if (comp.product.category === "inverter")
          iKva += (comp.product.rating_kva || 0) * qty;
        if (comp.product.category === "battery")
          bKwh += (comp.product.capacity_kwh || 0) * qty;
      });
      return {
        totalCost: cost,
  ...
  const handleSaveOrUpdate = () => {
    ...
    const request = editingTemplate
      ? axios.put(
          `${API_URL}/api/system_templates/${editingTemplate.id}`,
          payload
        )
    ...
    request
      .then((res) => {
        setSaveSuccess(
          `System "${templateName}" ${editingTemplate ? "updated" : "saved"} successfully!`
        );
        handleClear();
  ...
  const handleEditClick = (template) => {
    ...
    setExtrasCost(
      template.extras_cost !== undefined ? template.extras_cost.toString() : ""
    );

    // Reconstruct the components list with full product details
    const reconstructedComponents = template.components
      .map((comp) => {
  ...
  const handleDelete = (templateId) => {
    if (
      window.confirm("Are you sure you want to delete this system template?")
    ) {
      axios
        .delete(`${API_URL}/api/system_templates/${templateId}`)
        .then(() => {
          alert("Template deleted successfully!");
          fetchTemplates(); // Refresh list
        })
        .catch((err) =>
          alert("Failed to delete template: " + err.response?.data?.error)
        );
    }
  };
          ...
          <Col lg={7} xl={8}>
            <Card className="shadow-lg border-0 rounded-xl p-4 p-md-5">
              ...
              <p className="text-muted mb-4">
                Create new system kits for the Quick Design tool.
              </p>
              <Form>
                <Row>
                  <Col md={6}>
                    <Form.Group className="mb-3">
                      ...
                      <Form.Control
                        ...
                        placeholder="e.g., Commercial 50kW Hybrid"
                      />
                    </Form.Group>
                  </Col>
                  <Col md={6}>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Control
                    ...
                    placeholder="A brief description of the system's purpose or ideal use case."
                  />
                </Form.Group>
              </Form>
            ...
            >
              ...
              <Card className="shadow-lg border-0 rounded-xl mb-4">
                <Card.Header className="bg-dark text-white rounded-top-xl py-3">
                  <h3 className="text-xl font-semibold mb-0">
                    <i className="bi bi-card-checklist me-2"></i>System Summary
                  </h3>
                </Card.Header>
                <Card.Body className="p-4">
                  {components.length === 0 ? (
                    <p className="text-muted text-center py-4">
                      Add products from the left to build your system.
                    </p>
                  ) : (
                    components.map((comp) => (
                      <div
                      ...
                      >
                        <div className="flex-grow-1">
                          <p className="mb-0 fw-bold text-sm text-gray-800">
                            {comp.product.brand} {comp.product.model}
                          </p>
                          <div className="d-flex justify-content-between align-items-center">
                            <p className="mb-0 text-xs text-muted">
                              R{comp.product.price?.toLocaleString()} ea.
                            </p>
                        ...
                        <InputGroup style={{ width: "120px" }} className="ms-2">
                          ...
                          <Form.Control
                            ...
                            value={comp.quantity}
                            onChange={(e) =>
                              updateQuantity(comp.product.id, e.target.value)
                            }
                            onBlur={(e) =>
                              handleQuantityBlur(
                                comp.product.id,
                  ...
                  <div className="d-grid gap-2 mt-4">
                    ...
                    >
                      {isSaving ? (
                        ...
                        <>
                          <i className="bi bi-save-fill me-2"></i>Save System
                        </>
                      )}
                    </Button>
              ...
              <Card className="shadow-lg border-0 rounded-xl">
                <Card.Header className="bg-secondary text-white rounded-top-xl py-3">
                  <h3 className="text-xl font-semibold mb-0">
                    <i className="bi bi-collection-fill me-2"></i>Existing
                    Systems
                  </h3>
                </Card.Header>
                      ...
                      >
                        <div>
                          ...
                          <small className="text-muted">
                            {template.panel_kw}kWp | {template.inverter_kva}kVA
                            | {template.battery_kwh}kWh
                          </small>
                        </div>
