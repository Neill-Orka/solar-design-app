...
export default function ProjectQuoteDetails() {
  ...
  const handleLoadToBOM = async (versionId) => {
    try {
      const response = await axios.post(
        `${API_URL}/api/quote-versions/${versionId}/load-to-bom`
      );
      ...
      navigate(
        `/projects/${projectId}?tab=bom&quoteDoc=${doc.id}&quoteNo=${doc.number}&fromVersion=${vDetail.version_no}`
      );
    } catch (e) {
      console.error(e);
  ...
  const handleSendQuote = async () => {
    ...
    try {
      ...
      if (vDetail && vDetail.line_items) {
        console.log(
          "vDetail found, processing line items:",
          vDetail.line_items
        );
        ...
        console.log(
          "Navigating to:",
          `/projects/${projectId}/printable-bom/${docId}?action=download`
        );
      ...
      console.error(e);
      showNotification(
        e.response?.data?.error || "Failed to send quote",
        "danger"
      );
    } finally {
  ...
  const handleMarkAccepted = async () => {
    ...
    try {
      ...
      console.error(e);
      showNotification(
        e.response?.data?.error || "Failed to mark quote as accepted",
        "danger"
      );
    } finally {
      setActionLoading(false);
  ...
  const handleMarkDeclined = async () => {
    ...
    try {
      ...
      console.error(e);
      showNotification(
        e.response?.data?.error || "Failed to mark quote as declined",
        "danger"
      );
    } finally {
      setActionLoading(false);
  ...
  useEffect(() => {
    ...
    setVLoading(true);
    axios
      .get(`${API_URL}/api/quote-versions/${selectedVid}`)
      .then((r) => {
        if (!mounted) return;
        setVDetail(r.data);
        setVLoading(false);
  ...
  return (
    <Container fluid className="py-3">
      <Row className="align-items-center mb-3">
        <Col>
          <h5 className="mb-0">
            Quote <span className="text-muted">{doc.number}</span>
          </h5>
              ...
              {!vLoading && vDetail && (
                <>
                  <div className="mb-2">
                    <Badge
                      bg={
                        isDraft ? "secondary" : isSent ? "primary" : "success"
                      }
                      className="me-2"
                    >
                    ...
                    <span className="text-muted">
                      {new Date(vDetail.created_at).toLocaleString("en-ZA")}
                    </span>
                  </div>
                  {vDetail.valid_until && (
                    <div className="small text-muted mb-2">
                      Valid until:{" "}
                      {new Date(vDetail.valid_until).toLocaleDateString(
                        "en-GB"
                      )}
                    </div>
                  )}
                  <div className="small text-muted">
                    {vDetail.pdf_path ? (
                      ...
                      <span className="text-muted">
                        PDF will be generated when sent
                      </span>
                    ) : null}
                  </div>
                </>
              ...
              {!vLoading && vDetail && (
                <Row>
                  ...
                  <Col xs={6} className="text-end mb-2">
                    {fmtZAR(vDetail.totals?.subtotal_items_excl_vat)}
                  </Col>
                  ...
                  <Col xs={6} className="text-end mb-2">
                    {fmtZAR(vDetail.totals?.extras_excl_vat)}
                  </Col>
                  ...
                  <Col xs={6} className="text-end mb-2">
                    {fmtZAR(vDetail.totals?.total_excl_vat)}
                  </Col>

                  <Col xs={6} className="mb-2">
                    VAT ({vDetail.totals?.vat_perc ?? 15}%):
                  </Col>
                  <Col xs={6} className="text-end mb-2">
                    {fmtZAR(vDetail.totals?.vat_price)}
                  </Col>
                  ...
                  <Col xs={6} className="text-end">
                    <b>{fmtZAR(vDetail.totals?.total_incl_vat)}</b>
                  </Col>
                </Row>
              )}
            </Card.Body>
          ...
          <Col>
            <Card>
              ...
              <Card.Body>
                {isDraft && (
                  <div className="d-flex gap-2 flex-wrap">
                    <Button
                      ...
                      onClick={() => handleLoadToBOM(selectedVid)}
                ...
                {(currentVersionStatus === "accepted" ||
                  currentVersionStatus === "declined") && (
                  <Alert
                    variant={
                      currentVersionStatus === "accepted"
                        ? "success"
                  ...
                  >
                    <i
                      className={`bi ${currentVersionStatus === "accepted" ? "bi-check-circle" : "bi-x-circle"} me-1`}
                    ></i>
                  ...
                  <tr key={li.id}>
                    <td>
                      <div className="fw-semibold">
                        {li.brand || li.product_snapshot_json?.brand || "—"}{" "}
                        {li.model || li.product_snapshot_json?.model || ""}
                      </div>
                      <div className="small text-muted">
                        ID: {li.product_id || "n/a"}
                      </div>
                    </td>
                    <td>{li.qty}</td>
      ...
      <div className="mt-3 d-flex justify-content-between">
        <Link to={`/projects/${projectId}?tab=quotes`}>
          <Button variant="outline-secondary">Back to Quotes</Button>
        </Link>
