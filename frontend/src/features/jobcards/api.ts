...
export async function approveJobCard(
  id: number,
  options?: { invoice?: boolean; comment?: string }
) {
...
export async function updateTechnician(
  id: number,
  tech: Partial<TechnicianProfile>
): Promise<TechnicianProfile> {
...
export async function getAcceptedQuotes(projectId: number): Promise<any[]> {
  const { data } = await http.get(
    `/jobcards/projects/${projectId}/accepted_quotes`
  );
  return data;
}
...
export async function createInvoiceForJobCard(
  id: number,
  payload?: { type?: "deposit" | "delivery" | "final"; note?: string }
): Promise<InvoiceRef> {
  const { data } = await http.post(`/jobcards/${id}/invoices`, {
