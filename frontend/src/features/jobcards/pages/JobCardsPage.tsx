...
const REVIEW_KEYS: JobCardReviewStatus[] = ["open", "submitted", "invoiced", "completed"];
...
export default function JobCardsPage() {
  ...
  const [scope, setScope] = useState<'mine'|'bum'|'all'>(() => (user.is_bum ? 'bum' : 'mine'));
        ...
        <div className="kb-col-list">
          {items.map((jc) => (
            ...
            >
              ...
              {jc.bum_comment && <div className="kb-card-body">{jc.bum_comment}</div>}
              <div className="kb-card-actions">
                <Link to={`/jobcards/${jc.id}`} className="kb-btn kb-btn-light">
                  View
                </Link>
                {canModerate && k !== "submitted" && (
                  <button className="kb-btn kb-btn-accept" onClick={() => onQuickChange(jc.id, "submitted")}>Submitted</button>
                )}
                {canModerate && k !== "invoiced" && (
                  <button className="kb-btn kb-btn-warn" onClick={() => onQuickChange(jc.id, "invoiced")}>Invoiced</button>
                )}
                {canModerate && k !== "completed" && (
                  <button className="kb-btn kb-btn-danger" onClick={() => onQuickChange(jc.id, "completed")}>Completed</button>
                )}
              </div>
            </div>
          ))}
          ...
          <div className="jc-left">
            <div className="jcFilters">
              {user.is_bum ? (
                ...
                <div className="jcSeg readonly" title="Technicians see only their own job cards">
                  My job cards
                </div>
              )}
