...
import type { JobCardUpsert } from "../types";
...
export default function JobCardCreatePage() {
  ...
  const onSubmit = async (values: JobCardFormValues, materialData?: any) => {
    try {
      ...
      const created = await createJobCard(values as unknown as JobCardUpsert);
      ...
      if (values.materials_used && materialData?.materialLines?.length > 0) {
        const {
          ...
          materialLines: any[];
          usedMaterials: Record<number, boolean>;
          materialFileUploads: Record<number, File | null>;
        } = materialData;

        // Only save materials marked as used
        const usedLines = materialLines.filter(
          (_: any, index: number) => usedMaterials[index] ?? true
        );

        for (let i = 0; i < usedLines.length; i++) {
          const line = usedLines[i];
          const originalIndex = materialLines.findIndex((l: any) => l === line);
      ...
      // 4. Navigate based on status: drafts go back to edit, submitted go to detail
      if (isDraft) {
        nav(`/jobcards/${created.id}/edit`, { replace: true });
      } else {
