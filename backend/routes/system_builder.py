        ...
        for template in templates:
            ...
            for comp in template.components:
                if comp.product:
                    ...
                    if cat == "panel":
                        panel_kw += (
                            (comp.product.power_rating_w or 0) * comp.quantity / 1000
                        )
                    elif cat == "inverter":
                        inverter_kva += (
                            comp.product.power_rating_kva or 0
                        ) * comp.quantity
                    elif cat == "battery":
        ...
        for comp_data in data.get("components", []):
            ...
            if not product:
                ...
                return (
                    jsonify(
                        {
                            "error": f"Product with ID {comp_data.get('product_id')} not found."
                        }
                    ),
                    404,
                )
...
def delete_system_template(template_id):
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "message": "System template deleted successfully"}
            ),
            200,
        )
